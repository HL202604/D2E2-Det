...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max, use_dfl=False):
        ...
        if self.use_wiseiou:
            self.wiou_loss = WiseIouLoss(ltype='PIoU', monotonous=False, inner_iou=False, focaler_iou=False)
...
class DepthAwareDetectionLoss(v8DetectionLoss):
    """Depth-aware YOLOv8 loss function"""

    def __init__(self, model):
        super().__init__(model)
        self.mse = nn.MSELoss()  # 添加MSE损失
        self.depth_loss_weight = self.hyp.get('depth', 0.5)  # 从hyp获取参数

    def __call__(self, preds, batch):
        # 原始损失计算
        total_loss, loss = super().__call__(preds, batch)

        # 添加深度损失 ------------------------------------------------------
        if 'depths' in batch and 'pads' in batch:
            feats = preds[1] if isinstance(preds, tuple) else preds
            pred_distri = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2)[:, :self.reg_max * 4, :]

            # 解码预测框 (保留梯度)
            anchor_points, _ = make_anchors(feats, self.stride, 0.5)
            pred_boxes = self.bbox_decode(anchor_points, pred_distri)  # [BS, num_anchors, 4]

            # 获取目标框
            targets = torch.cat((batch["batch_idx"].view(-1, 1), batch["cls"].view(-1, 1), batch["bboxes"]), 1)
            targets = self.preprocess(targets.to(self.device), pred_boxes.size(0),
                                      scale_tensor=torch.tensor(feats[0].shape[2:], device=self.device) * self.stride[0])

            # 计算深度损失
            depth_loss = self._compute_depth_loss(
                pred_boxes,
                targets[..., 2:].clone(),  # 真实框坐标
                batch['depths'],
                batch['pads']
            )
            ...
            # 按论文公式组合损失: L = L_box + L_cls + L_dfl + λ*L_depth
            total_loss += depth_loss * self.depth_loss_weight
            loss = torch.cat((loss, depth_loss.unsqueeze(0)))

            # 输出每个损失分量
            print(f"Total Loss (after depth adjustment): {total_loss.item()}")
            print(f"Loss details - Box Loss: {loss[0].item()}, Class Loss: {loss[1].item()}, DFL Loss: {loss[2].item()}, Depth Loss: {loss[3].item()}")

        return total_loss, loss

    def _compute_depth_loss(self, pred_boxes, target_boxes, depths, pads):
        """向量化深度损失计算"""
        # 坐标转换 (保持梯度)
        pred_xyxy = xywh2xyxy(pred_boxes) * self.stride[0]  # [BS, N, 4]
        target_xyxy = xywh2xyxy(target_boxes) * self.stride[0]

        # 计算MBR区域
        min_xy = torch.minimum(pred_xyxy[..., :2], target_xyxy[..., :2])
        max_xy = torch.maximum(pred_xyxy[..., 2:], target_xyxy[..., 2:])
        ...
        for b in range(len(depths)):
            # 获取当前batch的填充参数
            pad_w, pad_h = pads[b]
            img_h, img_w = depths[b].shape[-2:]

            # 有效区域计算
            valid_area = torch.tensor([
                [pad_w, pad_h, img_w - pad_w, img_h - pad_h]
            ], device=self.device)

            # 坐标截断
            pred_valid = torch.clamp(pred_xyxy[b], valid_area[0, 0], valid_area[0, 2])
            target_valid = torch.clamp(target_xyxy[b], valid_area[0, 0], valid_area[0, 2])

            # MBR区域提取
            mbr_min = torch.minimum(pred_valid[..., :2], target_valid[..., :2])
            mbr_max = torch.maximum(pred_valid[..., 2:], target_valid[..., 2:])

            # 批量提取深度特征
            depth_maps = depths[b].unsqueeze(1)  # [C, H, W] -> [1, C, H, W]
            pred_masks = self._create_box_mask(mbr_min, mbr_max, depth_maps.shape)
            target_masks = self._create_box_mask(target_valid[..., :2], target_valid[..., 2:], depth_maps.shape)

            # 计算损失
            depth_loss += self.mse(depth_maps * pred_masks, depth_maps * target_masks)

        # 输出每个batch的深度损失
        print(f"Depth Loss for this batch: {depth_loss.item()}")

        return depth_loss / len(depths)

    def _create_box_mask(self, starts, ends, shape):
        """创建批量框mask"""
        masks = torch.zeros(shape, device=self.device)
