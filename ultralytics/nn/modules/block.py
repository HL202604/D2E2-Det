...
from .transformer import TransformerBlock, DynamicTanh
...
class ADD1(nn.Module):
    # Add two tensors after dividing by a learnable parameter C

    def __init__(self, arg):
        super(ADD1, self).__init__()
        # 初始化一个可学习的参数 C，随机初始化
        self.C = nn.Parameter(torch.rand(1))  # C为单一的可学习参数
    ...
    def forward(self, x):
        ...
        A_div_C = A / self.C
        B_div_C = B / self.C
        return torch.add(A_div_C, B_div_C)
...
#     def __init__(self, c_hsi, c_msi=None, reduction_ratio=4):
#         """
#         最终稳定版跨模态注意力融合模块
#         Args:
#             c_hsi: HSI模态通道数（必须参数）
#             c_msi: MSI模态通道数（若未提供则等于c_hsi）
#             reduction_ratio: 通道压缩比（必须>=2）
#         """
#         super().__init__()
#         c_msi = c_hsi if c_msi is None else c_msi
#
#         # 参数安全检查
#         assert reduction_ratio >= 2, f"缩减比例必须≥2, 当前为{reduction_ratio}"
#         assert c_hsi % reduction_ratio == 0, f"HSI通道数{c_hsi}必须能被{reduction_ratio}整除"
#         assert c_msi % reduction_ratio == 0, f"MSI通道数{c_msi}必须能被{reduction_ratio}整除"
#
#         # 模态对齐投影
#         self.proj_hsi = nn.Conv2d(c_hsi, c_hsi, 1, bias=False)
#         self.proj_msi = nn.Conv2d(c_msi, c_msi, 1, bias=False)
#
#         # HSI->MSI分支
#         self.query_h = nn.Conv2d(c_hsi, c_hsi // reduction_ratio, 1)
#         self.key_m = nn.Conv2d(c_msi, c_hsi // reduction_ratio, 1)
#         self.value_h = nn.Conv2d(c_hsi, c_hsi, 1)
#
#         # MSI->HSI分支
#         self.query_m = nn.Conv2d(c_msi, c_msi // reduction_ratio, 1)
#         self.key_h = nn.Conv2d(c_hsi, c_msi // reduction_ratio, 1)
#         self.value_m = nn.Conv2d(c_msi, c_msi, 1)
#
#         # 残差系数
...
#         for conv in [self.query_h, self.key_m, self.query_m, self.key_h]:
#             nn.init.kaiming_normal_(conv.weight, mode='fan_out', nonlinearity='relu')
#             if conv.bias is not None: nn.init.zeros_(conv.bias)
#
#         for conv in [self.value_h, self.value_m]:
#             nn.init.xavier_normal_(conv.weight)
#             if conv.bias is not None: nn.init.zeros_(conv.bias)
#
#     def forward(self, hsi, msi):
#         # 输入维度检查
#         assert hsi.shape[2:] == msi.shape[2:], f"空间维度不匹配: HSI{hsi.shape} vs MSI{msi.shape}"
#
#         # 模态对齐
#         h = self.proj_hsi(hsi)
#         m = self.proj_msi(msi)
...
#
#         # HSI→MSI注意力
#         Q_h = self.query_h(h).view(B, -1, H * W).permute(0, 2, 1)  # [B, N, C/r]
#         K_m = self.key_m(m).view(B, -1, H * W)  # [B, C/r, N]
#         attn = torch.bmm(Q_h, K_m) * (1.0 / (Q_h.size(-1) ** 0.5))
#         attn = F.softmax(attn, dim=-1)  # [B, N, N]
#         V_h = self.value_h(h).view(B, -1, H * W)  # [B, C, N]
#         F_h2m = torch.bmm(V_h, attn.permute(0, 2, 1)).view_as(h)
#
#         # MSI→HSI注意力
#         Q_m = self.query_m(m).view(B, -1, H * W).permute(0, 2, 1)  # [B, N, C/r]
#         K_h = self.key_h(h).view(B, -1, H * W)  # [B, C/r, N]
#         attn = torch.bmm(Q_m, K_h) * (1.0 / (Q_m.size(-1) ** 0.5))
#         attn = F.softmax(attn, dim=-1)  # [B, N, N]
#         V_m = self.value_m(m).view(B, -1, H * W)  # [B, C, N]
#         F_m2h = torch.bmm(V_m, attn.permute(0, 2, 1)).view_as(m)
#
#         # 残差连接
#         return self.gamma_h * F_m2h + h, self.gamma_m * F_h2m + m
...
#
#         # 动态计算注意力头数 (兼容不同通道)
#         self.heads = max(4, c_hsi // 64)  # 保证通道能被头数整除
#         self.dim_head = c_hsi // self.heads
#
#         # 自注意力预处理
#         self.self_attn_rgb = nn.Conv2d(c_hsi, c_hsi, 3, padding=1, groups=c_hsi)
#         self.self_attn_depth = nn.Conv2d(c_msi, c_msi, 3, padding=1, groups=c_msi)
#
#         # 交叉注意力投影层
#         self.to_q1 = nn.Conv2d(c_hsi, c_hsi, 1)
#         self.to_k1 = nn.Conv2d(c_msi, c_hsi, 1)
#         self.to_v1 = nn.Conv2d(c_msi, c_hsi, 1)
#
#         self.to_q2 = nn.Conv2d(c_msi, c_msi, 1)
#         self.to_k2 = nn.Conv2d(c_hsi, c_msi, 1)
#         self.to_v2 = nn.Conv2d(c_hsi, c_msi, 1)
#
#         # 差分注意力参数
#         self.lambda_rgb = nn.Parameter(torch.tensor(0.5))
#         self.lambda_depth = nn.Parameter(torch.tensor(0.5))
#
#         # 动态融合门控（保持输出通道与输入一致）
#         self.gate_rgb = nn.Conv2d(c_hsi * 2, c_hsi, 1)
#         self.gate_depth = nn.Conv2d(c_msi * 2, c_msi, 1)
#
#     def forward(self, rgb, depth):
#         B, C, H, W = rgb.shape
#
#         # 阶段1：自注意力增强（保持4D输入）
#         rgb_self = self.self_attn_rgb(rgb) + rgb  # [B,C,H,W]
#         depth_self = self.self_attn_depth(depth) + depth
#
#         # 阶段2：交叉注意力计算
#         # 生成四组投影特征（保持空间维度）
#         Q1 = self._project(self.to_q1(rgb_self))  # [B,heads,dim_head,H,W]
#         K1 = self._project(self.to_k1(depth_self))
#         V1 = self._project(self.to_v1(rgb_self))
#
#         Q2 = self._project(self.to_q2(depth_self))
#         K2 = self._project(self.to_k2(rgb_self))
#         V2 = self._project(self.to_v2(depth_self))
#
#         # 标准交叉注意力
#         sim_rgb = self._spatial_attn(Q1, K1, V1)  # [B,C,H,W]
#         sim_depth = self._spatial_attn(Q2, K2, V2)
#
#         # 差分交叉注意力
#         diff_rgb = self._diff_attn(Q1, K2, V1, self.lambda_rgb)
#         diff_depth = self._diff_attn(Q2, K1, V2, self.lambda_depth)
#
#         # 动态门控融合（维度对齐关键步骤）
#         fused_rgb = self.gate_rgb(torch.cat([sim_rgb, diff_rgb], dim=1)) + rgb
#         fused_depth = self.gate_depth(torch.cat([sim_depth, diff_depth], dim=1)) + depth
#
...
#         """将特征投影到多头空间（保持4D输入）"""
#         return rearrange(x,
#                          'b (h d) hh ww -> b h d hh ww',
#                          h=self.heads,
#                          d=self.dim_head)
#
#     def _spatial_attn(self, Q, K, V):
#         """空间注意力计算（保持4D特征）"""
#         attn = torch.einsum('b h d x y, b h d u v -> b h x y u v', Q, K).softmax(dim=-1)
#         out = torch.einsum('b h x y u v, b h d u v -> b h d x y', attn, V)
#         return rearrange(out, 'b h d x y -> b (h d) x y')  # 合并头维度
#
#     def _diff_attn(self, Q, K, V, lambda_):
#         """差分注意力计算"""
#         attn1 = self._spatial_attn(Q, K, V)
#         attn2 = self._spatial_attn(Q, K.roll(shifts=2, dims=-1), V)  # 位移增强
#         return attn1 - lambda_ * attn2

#scaled_dot_product_attention实现交叉注意力版本
...
class CrossModalAttentionFusion(nn.Module):
    def __init__(self, c_hsi, c_msi=None, reduction_ratio=4):
        ...
        # HSI->MSI分支 (Q=HSI, K=MSI, V=HSI)
        self.query_h = nn.Conv2d(c_hsi, c_msi // reduction_ratio, 1)  # 输出目标模态的缩减通道
        self.key_m = nn.Conv2d(c_msi, c_msi // reduction_ratio, 1)
        self.value_h = nn.Conv2d(c_hsi, c_msi, 1)  # HSI→MSI通道转换

        # MSI->HSI分支 (Q=MSI, K=HSI, V=MSI)
        self.query_m = nn.Conv2d(c_msi, c_hsi // reduction_ratio, 1)  # 输出目标模态的缩减通道
        self.key_h = nn.Conv2d(c_hsi, c_hsi // reduction_ratio, 1)
        self.value_m = nn.Conv2d(c_msi, c_hsi, 1)  # MSI→HSI通道转换

        # 可学习残差系数
        self.gamma_h = nn.Parameter(torch.zeros(1))  # HSI分支缩放
        self.gamma_m = nn.Parameter(torch.zeros(1))  # MSI分支缩放
    ...
    def forward(self, hsi, msi):
        # 空间维度检查
        assert hsi.shape[2:] == msi.shape[2:], \
            f"空间维度不匹配: HSI {hsi.shape} vs MSI {msi.shape}"

        # 模态对齐投影
        h = self.proj_hsi(hsi)  # [B, C_h, H, W]
        m = self.proj_msi(msi)  # [B, C_m, H, W]
        B, _, H, W = h.shape

        ''' 分支1: HSI→MSI注意力 (Q=HSI, K=MSI, V=HSI) '''
        # 生成Q, K, V
        Q_h = self.query_h(h)  # [B, C_m//r, H, W]
        K_m = self.key_m(m)  # [B, C_m//r, H, W]
        V_h = self.value_h(h)  # [B, C_m, H, W]

        # 重塑为注意力需要的形状 (B, N, C)
        Q_h = Q_h.flatten(2).permute(0, 2, 1)  # [B, N, C_m//r]
        K_m = K_m.flatten(2).permute(0, 2, 1)  # [B, N, C_m//r]
        V_h = V_h.flatten(2).permute(0, 2, 1)  # [B, N, C_m]

        # 使用PyTorch内置的高效注意力实现
        F_h2m = scaled_dot_product_attention(
            Q_h, K_m, V_h,
            attn_mask=None,
            dropout_p=0.0,
            is_causal=False
        ...
        # 恢复空间维度
        F_h2m = F_h2m.permute(0, 2, 1).view(B, -1, H, W)

        ''' 分支2: MSI→HSI注意力 (Q=MSI, K=HSI, V=MSI) '''
        Q_m = self.query_m(m)  # [B, C_h//r, H, W]
        K_h = self.key_h(h)  # [B, C_h//r, H, W]
        V_m = self.value_m(m)  # [B, C_h, H, W]

        # 重塑为注意力需要的形状
        Q_m = Q_m.flatten(2).permute(0, 2, 1)  # [B, N, C_h//r]
        K_h = K_h.flatten(2).permute(0, 2, 1)  # [B, N, C_h//r]
        V_m = V_m.flatten(2).permute(0, 2, 1)  # [B, N, C_h]

        F_m2h = scaled_dot_product_attention(
            Q_m, K_h, V_m,
            attn_mask=None,
            dropout_p=0.0,
            is_causal=False
        ...
        F_m2h = F_m2h.permute(0, 2, 1).view(B, -1, H, W)

        # 带缩放因子的残差连接
        # 注意：F_h2m增强MSI，F_m2h增强HSI
        h_fused = h + self.gamma_h * F_m2h
        m_fused = m + self.gamma_m * F_h2m

        return h_fused, m_fused
#SA+CA
class SA_CA(nn.Module):
    def __init__(self, c_hsi, c_msi=None, reduction_ratio=4, num_heads=8):
        """
        增强版跨模态注意力融合模块
        Args:
            c_hsi: HSI模态通道数
            c_msi: MSI模态通道数（默认等于c_hsi）
        ...
        assert c_hsi % reduction_ratio == 0, f"HSI通道数{c_hsi}必须能被{reduction_ratio}整除"
        assert c_msi % reduction_ratio == 0, f"MSI通道数{c_msi}必须能被{reduction_ratio}整除"
        assert c_hsi % num_heads == 0, f"HSI通道数{c_hsi}必须能被头数{num_heads}整除"
        assert c_msi % num_heads == 0, f"MSI通道数{c_msi}必须能被头数{num_heads}整除"
        ...
        # ================== 自注意力模块 ==================
        # HSI自注意力
        self.self_attn_hsi = nn.MultiheadAttention(
            embed_dim=c_hsi,
            num_heads=num_heads,
            batch_first=True
        )
        # MSI自注意力
        self.self_attn_msi = nn.MultiheadAttention(
            embed_dim=c_msi,
            num_heads=num_heads,
            batch_first=True
        )
        # 自注意力残差系数
        self.gamma_self_h = nn.Parameter(torch.zeros(1))
        self.gamma_self_m = nn.Parameter(torch.zeros(1))

        # ================== 交叉注意力模块 ==================
        # HSI->MSI分支 (Q=HSI, K=MSI, V=HSI)
        self.query_h = nn.Conv2d(c_hsi, c_msi // reduction_ratio, 1)
        self.key_m = nn.Conv2d(c_msi, c_msi // reduction_ratio, 1)
        self.value_h = nn.Conv2d(c_hsi, c_msi, 1)  # HSI→MSI通道转换

        # MSI->HSI分支 (Q=MSI, K=HSI, V=MSI)
        self.query_m = nn.Conv2d(c_msi, c_hsi // reduction_ratio, 1)
        self.key_h = nn.Conv2d(c_hsi, c_hsi // reduction_ratio, 1)
        self.value_m = nn.Conv2d(c_msi, c_hsi, 1)  # MSI→HSI通道转换

        # 交叉注意力残差系数
        self.gamma_cross_h = nn.Parameter(torch.zeros(1))
        self.gamma_cross_m = nn.Parameter(torch.zeros(1))

        self._init_weights()

    def _init_weights(self):
        # 初始化自注意力层
        for attn in [self.self_attn_hsi, self.self_attn_msi]:
            # 初始化QKV投影
    ...
    def forward(self, hsi, msi):
        ...
        # ================== 1. 自注意力增强 ==================
        # HSI自注意力
        h_flat = h.flatten(2).permute(0, 2, 1)  # [B, N, C_h]
        h_self, _ = self.self_attn_hsi(
            h_flat, h_flat, h_flat,
            need_weights=False
        )
        h_self = h_self.permute(0, 2, 1).view_as(h)  # 恢复形状
        h = h + self.gamma_self_h * h_self

        # MSI自注意力
        m_flat = m.flatten(2).permute(0, 2, 1)  # [B, N, C_m]
        m_self, _ = self.self_attn_msi(
            m_flat, m_flat, m_flat,
            need_weights=False
        )
        m_self = m_self.permute(0, 2, 1).view_as(m)  # 恢复形状
        m = m + self.gamma_self_m * m_self
        ...
        # ================== 3. 残差连接 ==================
        # 带缩放因子的残差连接
        h_fused = h + self.gamma_cross_h * F_m2h
        m_fused = m + self.gamma_cross_m * F_h2m

        return h_fused, m_fused


#7.10版本，在7.7用Concat相加V1和V2基础上把lambda参数使用Sigmoid限制到(0，1)之内
class SA_DiffA(nn.Module):
    def __init__(self, c_hsi, c_msi=None, depth=1, num_heads=8):
        super().__init__()
        c_msi = c_hsi if c_msi is None else c_msi
        ...
        self.lambda_value = 0.8 - 0.6 * math.exp(-0.3 * (depth - 1))
        self.lambda_param = nn.Parameter(torch.tensor(
            math.log(self.lambda_value / (1 - self.lambda_value))  # inverse sigmoid
        ))

        self.query_h = nn.Conv2d(c_hsi, c_msi, 1)
        self.key_m = nn.Conv2d(c_msi, c_msi, 1)
        self.value_h = nn.Conv2d(c_hsi, c_msi, 1)

        self.query_m = nn.Conv2d(c_msi, c_hsi, 1)
        self.key_h = nn.Conv2d(c_hsi, c_hsi, 1)
        self.value_m = nn.Conv2d(c_msi, c_hsi, 1)

        self.v_fusion_conv = nn.Conv2d(c_msi + c_hsi, c_msi, 1)

    def forward(self, hsi, msi):
        lambda_param = torch.sigmoid(self.lambda_param)
        ...
        h_flat = h.flatten(2).permute(0, 2, 1)  # [B, N, C_h]
        h_self, _ = self.self_attn_hsi(h_flat, h_flat, h_flat, need_weights=False)
        h_self = h_self.permute(0, 2, 1).view_as(h)  # 恢复形状
        h = h + h_self  # 残差连接

        m_flat = m.flatten(2).permute(0, 2, 1)  # [B, N, C_m]
        m_self, _ = self.self_attn_msi(m_flat, m_flat, m_flat, need_weights=False)
        m_self = m_self.permute(0, 2, 1).view_as(m)  # 恢复形状
        m = m + m_self  # 残差连接

        ''' 分支1: HSI→MSI注意力 (Q=HSI, K=MSI, V=HSI) '''
        Q_h = self.query_h(h)  # [B, C_m, H, W]
        K_m = self.key_m(m)  # [B, C_m, H, W]
        V_h = self.value_h(h)  # [B, C_m, H, W]

        Q_h_seq = Q_h.flatten(2).permute(0, 2, 1)  # [B, N, C_m]
        K_m_seq = K_m.flatten(2).permute(0, 2, 1)  # [B, N, C_m]

        attn_h2m = torch.bmm(Q_h_seq, K_m_seq.transpose(1, 2)) / math.sqrt(K_m_seq.size(-1))
        attn_h2m = F.softmax(attn_h2m, dim=-1)

        Q_m = self.query_m(m)  # [B, C_h, H, W]
        K_h = self.key_h(h)  # [B, C_h, H, W]
        V_m = self.value_m(m)  # [B, C_h, H, W]

        Q_m_seq = Q_m.flatten(2).permute(0, 2, 1)  # [B, N, C_h]
        K_h_seq = K_h.flatten(2).permute(0, 2, 1)  # [B, N, C_h]

        attn_m2h = torch.bmm(Q_m_seq, K_h_seq.transpose(1, 2)) / math.sqrt(K_h_seq.size(-1))
        attn_m2h = F.softmax(attn_m2h, dim=-1)

        # 应用差分公式 (A1-λA2)×(V1+V2) 和 (A2-λA1)×(V2+V1)
        V_concat = torch.cat([
            V_h,  # [B, C_m, H, W]
            V_m  # [B, C_h, H, W]
        ], dim=1)  # [B, C_m + C_h, H, W]
        V_sum = self.v_fusion_conv(V_concat)  # [B, C_m, H, W]
        V_sum = V_sum.flatten(2).permute(0, 2, 1)  # [B, N, C_m]

        F_h2m = torch.bmm(attn_h2m - self.lambda_param * attn_m2h, V_sum)  # [B, N, C_m]
        F_m2h = torch.bmm(attn_m2h - self.lambda_param * attn_h2m, V_sum)  # [B, N, C_h]

        F_h2m = F_h2m.permute(0, 2, 1).view(B, -1, H, W)  # [B, C_m, H, W]
        F_m2h = F_m2h.permute(0, 2, 1).view(B, -1, H, W)  # [B, C_h, H, W]
...
class ECAFusion(nn.Module):
    def __init__(self, c1, c2=None, b=1, gamma=2):
        """
        严格遵循原始ECA结构的双模态适配版
        Args:
            c1: 模态1通道数 (必须)
        ...
        # 模态1的ECA分支
        self.eca1 = self._build_eca(c1, b, gamma)

        # 模态2的ECA分支
        self.eca2 = self._build_eca(self.c2, b, gamma)

        # 保持原始残差连接方式
        self.gamma = nn.Parameter(torch.zeros(1))

    def _build_eca(self, channels, b, gamma):
        """完全复现原始ECA结构"""
    ...
    def _apply_eca(self, eca_block, x):
        """严格遵循原始ECA的前向流程"""
        # 平均池化
        y = eca_block[0](x)  # [B,C,1,1]

        # 维度调整
        y = y.squeeze(-1).transpose(1, 2)  # [B,1,C]

        # 1D卷积
        y = eca_block[1](y)  # [B,1,C]
    ...
    def forward(self, x1, x2):
        ...
        """
        # 模态内增强
        x1_att = self._apply_eca(self.eca1, x1)
        x2_att = self._apply_eca(self.eca2, x2)

        # 交叉融合（保持原始残差形式）
        return (
            x1 + self.gamma * x2_att,
            x2 + self.gamma * x1_att
        )


class FCANetFusion(nn.Module):
    def __init__(self, c1: int, c2: int = None, reduction: int = 16, freq_ratio: float = 0.5):
        super().__init__()
        # 参数校验
        assert reduction > 0, f"缩减比例必须>0，当前为{reduction}"
        self.c2 = c1 if c2 is None else c2
        self.freq_ratio = freq_ratio

        # 确保通道数合法
        c1_reduced = max(1, c1 // reduction)
        c2_reduced = max(1, self.c2 // reduction)
        assert c1_reduced > 0, f"模态1缩减后通道数必须>0，当前为{c1}//{reduction}={c1_reduced}"
        assert c2_reduced > 0, f"模态2缩减后通道数必须>0，当前为{self.c2}//{reduction}={c2_reduced}"

        # 初始化参数层
        self.fc1 = nn.Sequential(
            nn.Conv2d(c1, c1_reduced, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(c1_reduced, c1, 1, bias=False),
            nn.Sigmoid()
        )
        self.fc2 = nn.Sequential(
            nn.Conv2d(self.c2, c2_reduced, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(c2_reduced, self.c2, 1, bias=False),
            nn.Sigmoid()
        )

        # 确保fc1和fc2层的权重类型为float32
        self.fc1[0].weight.data = self.fc1[0].weight.data.to(torch.float32)
        self.fc1[2].weight.data = self.fc1[2].weight.data.to(torch.float32)
        self.fc2[0].weight.data = self.fc2[0].weight.data.to(torch.float32)
        self.fc2[2].weight.data = self.fc2[2].weight.data.to(torch.float32)

        # 注册DCT权重
        self.register_buffer('dct_weights', self._get_dct_weights(8))

        # 确保dct_weights是float32类型，避免类型不匹配
        self.dct_weights = self.dct_weights.to(torch.float32)
    ...
    def _get_dct_weights(self, n: int) -> Tensor:
        """生成DCT基矩阵"""
        weights = torch.zeros(n * n, 1, n, n)  # 输入通道=1
        for k in range(n * n):
            i, j = k // n, k % n
            x = torch.arange(n, dtype=torch.float32)
            y = torch.arange(n, dtype=torch.float32)
            weights[k, 0] = torch.cos(math.pi * (x + 0.5) * i / n) * torch.cos(math.pi * (y + 0.5) * j / n)
        return weights
    ...
    def _frequency_attention(self, x: Tensor, fc_layer: nn.Module) -> Tensor:
        B, C, H, W = x.size()
        ...
        # 2. 自适应填充
        x_padded = F.pad(x, (0, pad_w, 0, pad_h)) if (pad_h + pad_w) > 0 else x

        # 3. 分块处理
        patches = x_padded.unfold(2, 8, 8).unfold(3, 8, 8)
        patches = patches.contiguous().view(-1, 1, 8, 8)


        # 4. DCT变换
        dct = F.conv2d(patches, self.dct_weights)
        dct = dct.view(B, C, 64, H_padded // 8, W_padded // 8)

        # 5. 基选择
        mask = self._select_bases(dct)
        selected_dct = dct * mask

        # 6. 特征重构
        reconstructed = F.conv_transpose2d(
            selected_dct.view(-1, 64, 1, 1),
            self.dct_weights,
            stride=8
        )

        # 7. 恢复原始尺寸
        reconstructed = reconstructed.view(B, C, H_padded // 8, W_padded // 8, 8, 8)
        reconstructed = reconstructed.permute(0, 1, 2, 4, 3, 5).contiguous()
        reconstructed = reconstructed.view(B, C, H_padded, W_padded)

        # 8. 裁剪填充部分
        reconstructed = reconstructed[:, :, :H, :W]

        return fc_layer(reconstructed) * x

    def _select_bases(self, dct: Tensor) -> Tensor:
        """两阶段基选择"""
        B, C, N, H, W = dct.shape
        k = int(N * self.freq_ratio)

        # 阶段1：通道重要性排序
        channel_imp = torch.mean(dct, dim=(3, 4))  # [B,C,N]
        ranks = torch.argsort(channel_imp, dim=-1, descending=True)  # [B,C,N]

        # 阶段2：生成匹配维度的索引
        # 生成批次和通道的网格索引
        b_indices, c_indices = torch.meshgrid(
            torch.arange(B, device=dct.device),
            torch.arange(C, device=dct.device),
            indexing='ij'
        )  # 形状均为 [B,C]

        # 扩展索引到 [B,C,k]
        b_indices = b_indices.unsqueeze(-1).expand(-1, -1, k)  # [B,C,k]
        c_indices = c_indices.unsqueeze(-1).expand(-1, -1, k)  # [B,C,k]
        rank_indices = ranks[..., :k]  # [B,C,k]

        # 设置掩码
        mask = torch.zeros(B, C, N, device=dct.device)
        mask[b_indices, c_indices, rank_indices] = 1.0

        # 扩展掩码到空间维度
        mask = mask.view(B, C, N, 1, 1).expand_as(dct)  # [B,C,N,H,W]
    ...
    def forward(self, x1: Tensor, x2: Tensor) -> tuple:
        ...
        # print(f"x1 dtype: {x1.dtype}, dct_weights dtype: {self.dct_weights.dtype}")
        # print(f"fc1 weight dtype: {self.fc1[0].weight.data.dtype}")
        # print(f"fc2 weight dtype: {self.fc2[0].weight.data.dtype}")

        # 模态内频率增强
        x1_att = self._frequency_attention(x1, self.fc1)
        x2_att = self._frequency_attention(x2, self.fc2)
...
class CrissCrossFusion(nn.Module):
    def __init__(self, c1: int, c2: int = None, reduction_ratio=8, heads=4):
        """
        Criss-Cross双模态注意力融合模块
        Args:
            c1: 模态1通道数
            c2: 模态2通道数 (默认=c1)
        ...
        self.c2 = c1 if c2 is None else c2
        self.heads = heads
        self.channel_ratio = reduction_ratio

        # 模态对齐投影
        self.proj1 = nn.Conv2d(c1, c1 // reduction_ratio, 1)
        self.proj2 = nn.Conv2d(self.c2, self.c2 // reduction_ratio, 1)

        # 跨模态注意力参数
        self.query1 = nn.Conv2d(c1 // reduction_ratio, c1 // reduction_ratio, 1)
        self.key2 = nn.Conv2d(self.c2 // reduction_ratio, self.c2 // reduction_ratio, 1)
        self.value2 = nn.Conv2d(self.c2, self.c2, 1)

        self.query2 = nn.Conv2d(self.c2 // reduction_ratio, self.c2 // reduction_ratio, 1)
        self.key1 = nn.Conv2d(c1 // reduction_ratio, c1 // reduction_ratio, 1)
        self.value1 = nn.Conv2d(c1, c1, 1)
    ...
    def _init_weights(self):
        for conv in [self.query1, self.key2, self.query2, self.key1]:
            nn.init.kaiming_normal_(conv.weight, mode='fan_out', nonlinearity='relu')
        for conv in [self.value1, self.value2]:
            nn.init.xavier_normal_(conv.weight)

    def _criss_cross_attention(self, query: Tensor, key: Tensor, value: Tensor) -> Tensor:
        """Criss-Cross注意力计算"""
        B, C, H, W = query.size()

        # 生成位置编码，传递query的通道数
        position = self._get_position_encoding(H, W, query.device, C)

        # 多头处理
        q = self._split_heads(query + position)  # [B, heads, C', H*W]
        k = self._split_heads(key + position)  # [B, heads, C', H*W]
        v = self._split_heads(value)  # [B, heads, C'', H*W]

        # 注意力计算
        energy = torch.matmul(q.permute(0, 1, 3, 2), k)  # [B, heads, H*W, H*W]
        energy = F.softmax(energy / math.sqrt(C), dim=-1)

        # 特征聚合
        out = torch.matmul(v, energy.permute(0, 1, 3, 2))  # [B, heads, C'', H*W]
        out = self._combine_heads(out).view(B, -1, H, W)
        return out

    def _split_heads(self, x: Tensor) -> Tensor:
        B, C, H, W = x.size()
        return x.view(B, self.heads, C // self.heads, H * W)

    def _combine_heads(self, x: Tensor) -> Tensor:
        B, _, C, N = x.size()
        return x.permute(0, 2, 1, 3).contiguous().view(B, C * self.heads, N)

    def _get_position_encoding(self, H: int, W: int, device, query_channels: int) -> Tensor:
        """生成位置编码并根据query的通道数进行扩展"""
        loc_h = torch.arange(H, device=device).view(1, 1, H, 1)
        loc_w = torch.arange(W, device=device).view(1, 1, 1, W)

        # 生成位置编码，通道数为2，代表H和W的位置编码
        position = torch.cat([loc_h.expand(-1, 1, H, W), loc_w.expand(-1, 1, H, W)], dim=1)

        # 通过repeat扩展通道数，使其与query的通道数一致
        return position.repeat(1, query_channels // 2, 1, 1)  # 这里通过repeat扩展到query_channels的通道数

    def forward(self, x1: Tensor, x2: Tensor) -> tuple:
        # 模态对齐
        x1_reduced = self.proj1(x1)  # [B,C/r,H,W]
        x2_reduced = self.proj2(x2)

        # 模态1→模态2注意力
        attn1 = self._criss_cross_attention(
            self.query1(x1_reduced),
            self.key2(x2_reduced),
            self.value2(x2)
        )

        # 模态2→模态1注意力
        attn2 = self._criss_cross_attention(
            self.query2(x2_reduced),
            self.key1(x1_reduced),
            self.value1(x1)
        )

        # 残差连接
        return (
            x1 + self.gamma1 * attn1,
            x2 + self.gamma2 * attn2
        )


class SKFusion(nn.Module):
    def __init__(self, c_main: int, c_secondary: int):
        super().__init__()
        # 通道对齐（关键修正：统一使用channel_align）
        self.channel_align = (
            nn.Sequential(
                nn.Conv2d(c_secondary, c_main, 1),
                nn.BatchNorm2d(c_main)  # 使用BatchNorm2d
            ) if c_main != c_secondary
            else nn.Identity()
        )

        # 多分支卷积（保持原有结构）
        self.conv3 = nn.Sequential(
            nn.Conv2d(c_main, c_main, 3, padding=1, groups=32),
            nn.BatchNorm2d(c_main)  # 使用BatchNorm2d
        )
        self.conv5 = nn.Sequential(
            nn.Conv2d(c_main, c_main, 5, padding=2, groups=32),
            nn.BatchNorm2d(c_main)  # 使用BatchNorm2d
        )
        ...
        self.attn = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(c_main, c_main, 1),  # 将输出通道数调整为c_main
            nn.BatchNorm2d(c_main),  # 使用BatchNorm2d
            nn.ReLU(),
            nn.Conv2d(c_main, c_main, 1),  # 将输出通道数调整为c_main
        )
    ...
    def forward(self, x_main: Tensor, x_secondary: Tensor) -> tuple:
        # 维度对齐处理
        x_secondary = self.channel_align(x_secondary)  # 关键修正：使用channel_align

        # 空间对齐（自动插值）
        if x_secondary.shape[2:] != x_main.shape[2:]:
            x_secondary = F.interpolate(
                x_secondary,
                size=x_main.shape[2:],
                mode='bilinear',
                align_corners=False
        ...
        # 特征融合
        fused = x_main + x_secondary

        # 多分支卷积
        feat3 = self.conv3(fused)
        feat5 = self.conv5(fused)
        feats = feat3 + feat5  # 直接将特征加在一起

        # 动态注意力
        attn = self.attn(fused)  # [B, C, 1, 1]

        # 加权特征
        weighted_feats = feats * attn  # [B, C, H, W]

        # 残差输出
        x_main_fused = x_main + self.gamma * weighted_feats
        x_secondary_fused = x_secondary + self.gamma * weighted_feats

        # 返回两个处理后的结果
        return x_main_fused, x_secondary_fused
...
class CBAMFusion(nn.Module):
    def __init__(self, c_main: int, c_secondary: int):
        super().__init__()
        # 通道对齐
        self.channel_align = nn.Conv2d(c_secondary, c_main, 1) if c_secondary != c_main else nn.Identity()

        # 注意力机制
        self.channel_att = ChannelAttentionModule(c_main * 2)  # 拼接后通道数
        self.spatial_att = SpatialAttentionModule()

        # 残差参数
        self.gamma = nn.Parameter(torch.zeros(1))

    def forward(self, x_main: torch.Tensor, x_secondary: torch.Tensor) -> tuple:
        # 通道对齐
        x_secondary = self.channel_align(x_secondary)

        # 空间对齐
        if x_secondary.shape[-2:] != x_main.shape[-2:]:
            x_secondary = F.interpolate(x_secondary, size=x_main.shape[-2:], mode='bilinear', align_corners=False)

        # 特征拼接
        fused = torch.cat([x_main, x_secondary], dim=1)

        # CBAM注意力
        channel_att = self.channel_att(fused)
        attented = fused * channel_att
        spatial_att = self.spatial_att(attented)
        attented = attented * spatial_att

        # 关键修正：正确拆分为双模态输出
        return (
                x_main + self.gamma * attented[:, :x_main.shape[1]],  # 主模态增强
                                      x_secondary + self.gamma * attented[:, x_main.shape[1]:]  # 次模态增强
        )
...
class DualAttentionFusion(nn.Module):
    def __init__(self, c_main: int, c_secondary: int):
        ...
        self.channel_align = (
            nn.Sequential(
                nn.Conv2d(c_secondary, c_main, 1),
                nn.BatchNorm2d(c_main),
                nn.ReLU(inplace=True)
            ) if c_main != c_secondary
            else nn.Identity()
        )
            # 双注意力核心模块
        self.pam = PositionAttention_DAF(c_main)
        self.cam = ChannelAttention_DAF(c_main)
        ...
        # 融合后处理
        self.fusion_norm = nn.BatchNorm2d(c_main)
        self._init_weights()
    ...
    def forward(self, x_main: torch.Tensor, x_secondary: torch.Tensor) -> tuple:
        ...
        x_secondary = self.channel_align(x_secondary)
        if x_secondary.shape[-2:] != x_main.shape[-2:]:
            x_secondary = F.interpolate(x_secondary, x_main.shape[-2:], mode='bilinear')

        # 注意力特征融合
        base_feat = x_main + x_secondary
        pam_feat = self.pam(base_feat)
        cam_feat = self.cam(base_feat)

        # 加权融合（带残差连接）
        fused_feat = self.gamma * pam_feat + self.beta * cam_feat
        fused_feat = self.fusion_norm(fused_feat)

        # 稳定残差输出
        x_main_fused = x_main + 0.2 * fused_feat
        x_secondary_fused = x_secondary + 0.2 * fused_feat

        return x_main_fused, x_secondary_fused


class PositionAttention_DAF(nn.Module):
    def __init__(self, in_dim):
        ...
        self.scale = (in_dim // 8) ** -0.5  # 缩放因子
        self._init_conv()

    def _init_conv(self):
        for conv in [self.query, self.key, self.value]:
            kaiming_normal_(conv.weight, mode='fan_out', nonlinearity='relu')
...
class ChannelAttention_DAF(nn.Module):
    def __init__(self, in_dim):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 通道注意力计算
        proj = x.view(B, C, -1)  # [B, C, N]
        energy = torch.bmm(proj, proj.permute(0, 2, 1)) * self.scale  # [B, C, C]
        attn = self.softmax(energy + 1e-8)  # 数值稳定性

        # 特征增强
        out = torch.bmm(attn, proj)  # [B, C, N]
        return out.view(B, C, H, W)


class CoTFusion(nn.Module):
    def __init__(self, c_main: int, c_secondary: int, k: int = 3):
        super().__init__()
        # ========== 通道对齐模块 ==========
        self.channel_align = (
            nn.Sequential(
            ...
            # ========== CoT核心参数 ==========
        self.k = k  # 卷积核尺寸
        self.d = max(c_main // 4, 1)  # 压缩维度，最小为1
        self.fused_channels = c_main * 2  # 融合后通道数

        # ========== Static Context模块 ==========
        self.static_conv = nn.Sequential(
            nn.Conv2d(c_main, c_main, k, padding=k // 2, groups=c_main),
            nn.BatchNorm2d(c_main),
            nn.ReLU(inplace=True)
        )
        # ========== Dynamic Context模块 ==========
        ...
        # Query分支
        self.query_conv = nn.Conv2d(c_main, self.d, 1)

        # Key分支（输入通道数需匹配展开后的维度）
        self.key_conv = nn.Conv2d(self.fused_channels * k * k, self.d, 1)

        # Value分支
        self.value_conv = nn.Conv2d(self.fused_channels * k * k, c_main, 1)
    ...
    def _init_weights(self):
        for m in [self.query_conv, self.key_conv, self.value_conv]:
            ...
            if m.bias is not None:
                nn.init.constant_(m.bias, 0)
        nn.init.constant_(self.static_conv[1].weight, 1)  # BN初始化为单位变换
    ...
    def forward(self, x_main: torch.Tensor, x_secondary: torch.Tensor) -> tuple:
        ...
        # ========== Static Context分支 ==========
        static_feat = self.static_conv(x_main) + x_main  # 残差连接

        # ========== Dynamic Context分支 ==========
        # 1. 特征融合与展开
        fused = torch.cat([x_main, x_secondary], dim=1)  # [B, 2c, H, W]
        B, C, H, W = fused.shape

        # 2. 展开局部上下文 (k x k邻域)
        unfolded = self.unfold(fused)  # [B, C*k*k, H*W]
        unfolded = unfolded.view(B, C * self.k * self.k, H, W)  # 重塑为4D [B, C*k², H, W]

        # 3. Key计算
        key = self.key_conv(unfolded)  # [B, d, H, W]
        key = key.view(B, self.d, H * W).permute(0, 2, 1)  # [B, H*W, d]

        # 4. Query计算
        query = self.query_conv(x_main).view(B, self.d, H * W).permute(0, 2, 1)  # [B, H*W, d]

        # 5. 注意力计算（带缩放防止溢出）
        energy = torch.bmm(query, key.transpose(1, 2)) * (self.d ** -0.5)
        attention = F.softmax(energy.clamp(max=50), dim=-1)  # [B, H*W, H*W]

        # 6. Value计算
        value = self.value_conv(unfolded)  # [B, c_main, H, W]
        value = value.view(B, -1, H * W)  # [B, c_main, H*W]

        # 7. 上下文聚合
        dynamic_feat = torch.bmm(value, attention.permute(0, 2, 1))  # [B, c_main, H*W]
        dynamic_feat = dynamic_feat.view(B, -1, H, W)  # [B, c_main, H, W]

        # ========== 特征融合 ==========
        cot_feat = self.gamma * static_feat + self.delta * dynamic_feat

        # ========== 残差连接 ==========
        x_main_fused = x_main + 0.2 * cot_feat
        x_secondary_fused = x_secondary + 0.2 * cot_feat
...
class C3k2_FDConv(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(
            C3k_FDConv(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_FDConv(self.c, self.c, shortcut, g) for _
            in range(n))
...
class C3k2_MambaOut_FDConv(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(
            C3k_MambaOut_FDConv(self.c, self.c, n, shortcut, g) if c3k else GatedFDConvBlock_BCHW(self.c) for _ in
            range(n))
...
class C3k2_PFDConv(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(
            C3k_PFDConv(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_PFDConv(self.c, self.c, shortcut, g) for
            _ in range(n))
